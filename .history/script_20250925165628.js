...
function createCard(item) {
    ...
    const statusText = item.status === "done" ? "Done" : "Waiting List";
    ...
    card.innerHTML = `
        ...
        <div class="subtitle wa-row">
            <strong>WA:</strong> 
            <span class="wa-number">${item.nomor_wa}</span>
            <button class="btn-copy" title="Copy Nomor WA" data-wa="${item.nomor_wa}">📋</button>
        </div>
        <div class="actions">
            ${doneButton}
...
// Event delegation tombol copy WA
listContainer.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-copy")) {
        const waNumber = e.target.dataset.wa.trim(); // hanya nomor WA
...
function fallbackCopy(text) {
    ...
    try {
        ...
        showToast("Gagal menyalin nomor WA", "error");
    }
