...
// Fetch data
async function fetchData(page = 1, limit = 10, keyword = "") {
    const url = keyword
...
async function loadData(page = 1, limit = 10, keyword = "") {
    showLoader();
    try {
        const json = await fetchData(page, limit, keyword);
        listContainer.innerHTML = "";
        if (json.success && json.data.length) {
            const fragment = document.createDocumentFragment();
            json.data.forEach(item => fragment.appendChild(createCard(item)));
...
confirmBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        if (json.success) {
            if (selectedAction === "done") markAsDone(selectedItem.no);
            else removeCard(selectedItem.no);
